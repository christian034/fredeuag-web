...
import { Cn as from, Ft as concatMap, Qt as filter, S as startWith, W as pairwise, Xn as pipe, Yn as Observable, bt as distinct, fn as map, j as scan, rn as timer, yt as distinctUntilChanged } from "./zipWith-BIPy9JuQ.js";
import { t as _asyncToGenerator } from "./asyncToGenerator-B0cJ8fiL.js";
import { B as getUA, C as Logger, D as FirebaseError, H as isCloudWorkstation, I as getDefaultEmulatorHostnameAndPort, K as isIndexedDBAvailable, N as deepEqual, Q as pingServer, R as getGlobal, S as LogLevel, X as isSafariOrWebkit, Y as isSafari, b as registerVersion, c as _getProvider, d as _registerComponent, f as _removeServiceInstance, h as getApp, j as createMockUserToken, n as SDK_VERSION, tt as updateEmulatorBanner, u as _isFirebaseServerApp, w as Component, z as getModularInstance } from "./index.esm2017-DghwDOiZ.js";
...
import { n as AuthInstances } from "./angular-fire-auth-DFzzNuQT.js";
...
var __PRIVATE_FirebaseAuthCredentialsProvider = class {
	...
	start(e, t) {
		...
		const __PRIVATE_awaitNextToken = () => {
			const t = r;
			e.enqueueRetryable(_asyncToGenerator(function* () {
...
*/ function __PRIVATE_ignoreIfPrimaryLeaseLoss(_x) {
	return __PRIVATE_ignoreIfPrimaryLeaseLoss2.apply(this, arguments);
}
function __PRIVATE_ignoreIfPrimaryLeaseLoss2() {
	__PRIVATE_ignoreIfPrimaryLeaseLoss2 = _asyncToGenerator(function* (e) {
...
*/ var __PRIVATE_SimpleDb = class __PRIVATE_SimpleDb {
	...
	*/ k(e) {
		...
		return _asyncToGenerator(function* () {
			return _this2.db || (__PRIVATE_logDebug(U, "Opening database:", _this2.name), _this2.db = yield new Promise(((t, n) => {
				const r = indexedDB.open(_this2.name, _this2.version);
				r.onsuccess = (e) => {
					...
					__PRIVATE_logDebug(U, "Database \"" + _this2.name + "\" requires upgrade from version:", e.oldVersion);
					const t = e.target.result;
					if (null !== _this2.L && _this2.L !== e.oldVersion) throw new Error(`refusing to open IndexedDB database due to potential corruption of the IndexedDB database data; this corruption could be caused by clicking the "clear site data" button in a web browser; try reloading the web page to re-initialize the IndexedDB database: lastClosedDbVersion=${_this2.L}, event.oldVersion=${e.oldVersion}, event.newVersion=${e.newVersion}, db.version=${t.version}`);
					_this2.B.q(t, r.transaction, e.oldVersion, _this2.version).next((() => {
						__PRIVATE_logDebug(U, "Database upgrade to version " + _this2.version + " complete");
					}));
				};
			})), _this2.db.addEventListener("close", ((e) => {
				_this2.L = e.target.version;
			}), { passive: !0 })), _this2.db.addEventListener("versionchange", ((e) => {
				var t;
				null === e.newVersion && (__PRIVATE_logWarn("Received \"versionchange\" event with newVersion===null; notifying the registered DatabaseDeletedListener, if any"), null === (t = _this2.databaseDeletedListener) || void 0 === t || t.call(_this2));
			}), { passive: !0 }), _this2.db;
		})();
	}
	...
	runTransaction(e, t, n, r) {
		...
		return _asyncToGenerator(function* () {
			...
			for (;;) {
				++s;
				try {
					_this3.db = yield _this3.k(e);
					const t = __PRIVATE_SimpleDbTransaction.open(_this3.db, e, i ? "readonly" : "readwrite", n), s = r(t).next(((e) => (t.v(), e))).catch(((e) => (t.abort(e), PersistencePromise.reject(e)))).toPromise();
					return s.catch((() => {})), yield t.D, s;
				} catch (e) {
					const t = e, n = "FirebaseError" !== t.name && s < 3;
					if (__PRIVATE_logDebug(U, "Transaction failed with error:", t.message, "Retrying:", n), _this3.close(), !n) return Promise.reject(t);
				}
			}
		})();
	}
...
var __PRIVATE_IndexBackfillerScheduler = class {
	...
	ne(e) {
		var _this4 = this;
		__PRIVATE_logDebug(W, `Scheduled in ${e}ms`), this.task = this.asyncQueue.enqueueAfterDelay("index_backfill", e, _asyncToGenerator(function* () {
			_this4.task = null;
			try {
				__PRIVATE_logDebug(W, `Documents written: ${yield _this4.te.re()}`);
			} catch (e) {
				__PRIVATE_isIndexedDbTransactionError(e) ? __PRIVATE_logDebug(W, "Ignoring IndexedDB error during index backfill: ", e) : yield __PRIVATE_ignoreIfPrimaryLeaseLoss(e);
			}
			yield _this4.ne(6e4);
		}));
	}
...
/** Implements the steps for backfilling indexes. */ var __PRIVATE_IndexBackfiller = class {
	...
	re(e = 50) {
		var _this5 = this;
		return _asyncToGenerator(function* () {
			return _this5.persistence.runTransaction("Backfill Indexes", "readwrite-primary", ((t) => _this5.ie(t, e)));
		})();
	}
...
var Ct = /* @__PURE__ */ new Uint8Array(0);
/**
...
*/ var __PRIVATE_LruScheduler = class {
	...
	Rr(e) {
		var _this6 = this;
		__PRIVATE_logDebug(xt, `Garbage collection scheduled in ${e}ms`), this.Ar = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection", e, _asyncToGenerator(function* () {
			_this6.Ar = null;
			try {
				yield _this6.localStore.collectGarbage(_this6.garbageCollector);
			} catch (e) {
				__PRIVATE_isIndexedDbTransactionError(e) ? __PRIVATE_logDebug(xt, "Ignoring IndexedDB error during garbage collection: ", e) : yield __PRIVATE_ignoreIfPrimaryLeaseLoss(e);
			}
			yield _this6.Rr(3e5);
		}));
	}
...
var __PRIVATE_IndexedDbPersistence = class __PRIVATE_IndexedDbPersistence {
	...
	*/ setNetworkEnabled(e) {
		var _this8 = this;
		this.networkEnabled !== e && (this.networkEnabled = e, this.Fi.enqueueAndForget(_asyncToGenerator(function* () {
			_this8.started && (yield _this8.Ki());
		})));
	}
	/**
	...
	*/ ts() {
		var _this9 = this;
		return _asyncToGenerator(function* () {
			if (_this9.isPrimary && !_this9.ns(_this9.ki, Bt)) {
				_this9.ki = Date.now();
				const e = yield _this9.runTransaction("maybeGarbageCollectMultiClientState", "readwrite-primary", ((e) => {
					const t = __PRIVATE_getStore(e, pe);
					return t.j().next(((e) => {
						const n = _this9.rs(e, Bt), r = e.filter(((e) => -1 === n.indexOf(e)));
						return PersistencePromise.forEach(r, ((e) => t.delete(e.clientId))).next((() => r));
					}));
				})).catch((() => []));
				if (_this9.Ui) for (const t of e) _this9.Ui.removeItem(_this9.ss(t.clientId));
			}
		})();
	}
	/**
	...
	*/ Hi(e) {
	...
	shutdown() {
		var _this10 = this;
		return _asyncToGenerator(function* () {
			_this10.ci = !1, _this10.us(), _this10.Li && (_this10.Li.cancel(), _this10.Li = null), _this10.cs(), _this10.ls(), yield _this10.$i.runTransaction("shutdown", "readwrite", [J, pe], ((e) => {
				const t = new __PRIVATE_IndexedDbTransaction(e, __PRIVATE_ListenSequence.ue);
				return _this10.Yi(t).next((() => _this10.es(t)));
			})), _this10.$i.close(), _this10.hs();
		})();
	}
	/**
...
*/
function __PRIVATE_localStoreHandleUserChange(_x3, _x4) {
	return __PRIVATE_localStoreHandleUserChange2.apply(this, arguments);
}
function __PRIVATE_localStoreHandleUserChange2() {
	__PRIVATE_localStoreHandleUserChange2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
		return yield n.persistence.runTransaction("Handle user change", "readonly", ((e) => {
			let r;
...
*/
function __PRIVATE_localStoreReleaseTarget(_x5, _x6, _x7) {
	return __PRIVATE_localStoreReleaseTarget2.apply(this, arguments);
}
function __PRIVATE_localStoreReleaseTarget2() {
	__PRIVATE_localStoreReleaseTarget2 = _asyncToGenerator(function* (e, t, n) {
		const r = __PRIVATE_debugCast(e), i = r.Fs.get(t), s = n ? "readwrite" : "readwrite-primary";
		try {
			n || (yield r.persistence.runTransaction("Release target", s, ((e) => r.persistence.referenceDelegate.removeTarget(e, i))));
		} catch (e) {
...
* queue.
*/
function __PRIVATE_localStoreApplyBundledDocuments(_x8, _x9, _x10, _x11) {
	return __PRIVATE_localStoreApplyBundledDocuments2.apply(this, arguments);
}
function __PRIVATE_localStoreApplyBundledDocuments2() {
	__PRIVATE_localStoreApplyBundledDocuments2 = _asyncToGenerator(function* (e, t, n, r) {
		...
		const _ = i.Os.newChangeBuffer({ trackRemovals: !0 }), a = yield __PRIVATE_localStoreAllocateTarget(i, function __PRIVATE_umbrellaTarget(e) {
...
*/
function __PRIVATE_localStoreSaveNamedQuery(_x12, _x13) {
	return __PRIVATE_localStoreSaveNamedQuery2.apply(this, arguments);
}
function __PRIVATE_localStoreSaveNamedQuery2() {
	__PRIVATE_localStoreSaveNamedQuery2 = _asyncToGenerator(function* (e, t, n = __PRIVATE_documentKeySet()) {
...
*/ var __PRIVATE_WebStorageSharedClientState = class {
	...
	start() {
		var _this11 = this;
		return _asyncToGenerator(function* () {
			const e = yield _this11.syncEngine.Ps();
			for (const t of e) {
				if (t === _this11.js) continue;
				...
				if (e) {
					...
					n && (_this11.Ys = _this11.Ys.insert(n.clientId, n));
				}
			}
			_this11._o();
			const t = _this11.storage.getItem(_this11.so);
			if (t) {
				const e = _this11.ao(t);
				e && _this11.uo(e);
			}
			for (const e of _this11.Zs) _this11.Hs(e);
			_this11.Zs = [], _this11.window.addEventListener("pagehide", (() => _this11.shutdown())), _this11.started = !0;
		})();
	}
	...
	Hs(e) {
		...
		if (t.storageArea === this.storage) {
			...
			this.Fi.enqueueRetryable(_asyncToGenerator(function* () {
				if (_this12.started) {
					if (null !== t.key) {
						if (_this12.no.test(t.key)) {
							if (null == t.newValue) {
								const e = _this12.Ao(t.key);
								return _this12.Ro(e, null);
							}
							{
								const e = _this12.Vo(t.key, t.newValue);
								if (e) return _this12.Ro(e.clientId, e);
							}
						} else if (_this12.ro.test(t.key)) {
							if (null !== t.newValue) {
								const e = _this12.mo(t.key, t.newValue);
								if (e) return _this12.fo(e);
							}
						} else if (_this12.io.test(t.key)) {
							if (null !== t.newValue) {
								const e = _this12.po(t.key, t.newValue);
								if (e) return _this12.yo(e);
							}
						} else if (t.key === _this12.so) {
							if (null !== t.newValue) {
								const e = _this12.ao(t.newValue);
								if (e) return _this12.uo(e);
							}
						} else if (t.key === _this12.eo) {
							...
							e !== __PRIVATE_ListenSequence.ue && _this12.sequenceNumberHandler(e);
						} else if (t.key === _this12.oo) {
							const e = _this12.So(t.newValue);
							yield Promise.all(e.map(((e) => _this12.syncEngine.bo(e))));
						}
					}
				} else _this12.Zs.push(t);
			}));
		}
	}
	get Po() {
	...
	*/ ao(e) {
	...
	fo(e) {
		var _this13 = this;
		return _asyncToGenerator(function* () {
			if (e.user.uid === _this13.currentUser.uid) return _this13.syncEngine.Do(e.batchId, e.state, e.error);
...
var __PRIVATE_PersistentStream = class {
	...
	*/ stop() {
		var _this14 = this;
		return _asyncToGenerator(function* () {
			_this14.M_() && (yield _this14.close(0));
		})();
	}
	/**
	...
	*/ B_() {
	...
	/** Called by the idle timer when the stream should close due to inactivity. */ L_() {
		var _this15 = this;
		return _asyncToGenerator(function* () {
			if (_this15.x_()) return _this15.close(0);
		})();
	}
	...
	*/ close(e, t) {
		var _this16 = this;
		return _asyncToGenerator(function* () {
			_this16.q_(), _this16.Q_(), _this16.F_.cancel(), _this16.b_++, 4 !== e ? _this16.F_.reset() : t && t.code === N.RESOURCE_EXHAUSTED ? (__PRIVATE_logError(t.toString()), __PRIVATE_logError("Using maximum backoff delay to prevent overloading the backend."), _this16.F_.f_()) : t && t.code === N.UNAUTHENTICATED && 3 !== _this16.state && (_this16.authCredentialsProvider.invalidateToken(), _this16.appCheckCredentialsProvider.invalidateToken()), null !== _this16.stream && (_this16.U_(), _this16.stream.close(), _this16.stream = null), _this16.state = e, yield _this16.listener.n_(t);
		})();
	}
	/**
	...
	*/ U_() {}
	...
	O_() {
		var _this17 = this;
		this.state = 5, this.F_.g_(_asyncToGenerator(function* () {
			_this17.state = 0, _this17.start();
		}));
	}
	G_(e) {
...
var __PRIVATE_RemoteStoreImpl = class {
	constructor(e, t, n, r, i) {
		...
		this.localStore = e, this.datastore = t, this.asyncQueue = n, this.remoteSyncer = {}, this.Pa = [], this.Ta = /* @__PURE__ */ new Map(), this.Ia = /* @__PURE__ */ new Set(), this.da = [], this.Ea = i, this.Ea.xo(((e) => {
			n.enqueueAndForget(_asyncToGenerator(function* () {
				__PRIVATE_canUseNetwork(_this18) && (__PRIVATE_logDebug(Xt, "Restarting streams for network reachability change."), yield (/* @__PURE__ */ function() {
					var __PRIVATE_restartNetwork2 = _asyncToGenerator(function* (e) {
						const t = __PRIVATE_debugCast(e);
						t.Ia.add(4), yield __PRIVATE_disableNetworkInternal(t), t.Aa.set("Unknown"), t.Ia.delete(4), yield __PRIVATE_enableNetworkInternal(t);
					});
					function __PRIVATE_restartNetwork(_x14) {
						return __PRIVATE_restartNetwork2.apply(this, arguments);
					}
...
function __PRIVATE_enableNetworkInternal(_x15) {
	return __PRIVATE_enableNetworkInternal2.apply(this, arguments);
}
function __PRIVATE_enableNetworkInternal2() {
	__PRIVATE_enableNetworkInternal2 = _asyncToGenerator(function* (e) {
		if (__PRIVATE_canUseNetwork(e)) for (const t of e.da) yield t(!0);
	});
	return __PRIVATE_enableNetworkInternal2.apply(this, arguments);
}
/**
...
* enableNetwork().
*/ function __PRIVATE_disableNetworkInternal(_x16) {
	return __PRIVATE_disableNetworkInternal2.apply(this, arguments);
}
function __PRIVATE_disableNetworkInternal2() {
	__PRIVATE_disableNetworkInternal2 = _asyncToGenerator(function* (e) {
		for (const t of e.da) yield t(!1);
	});
	return __PRIVATE_disableNetworkInternal2.apply(this, arguments);
}
/**
...
function __PRIVATE_onWatchStreamConnected(_x17) {
	return __PRIVATE_onWatchStreamConnected2.apply(this, arguments);
}
function __PRIVATE_onWatchStreamConnected2() {
	__PRIVATE_onWatchStreamConnected2 = _asyncToGenerator(function* (e) {
		e.Aa.set("Online");
	});
	return __PRIVATE_onWatchStreamConnected2.apply(this, arguments);
}
function __PRIVATE_onWatchStreamOpen(_x18) {
	return __PRIVATE_onWatchStreamOpen2.apply(this, arguments);
}
function __PRIVATE_onWatchStreamOpen2() {
	__PRIVATE_onWatchStreamOpen2 = _asyncToGenerator(function* (e) {
		e.Ta.forEach(((t, n) => {
...
function __PRIVATE_onWatchStreamClose(_x19, _x20) {
	return __PRIVATE_onWatchStreamClose2.apply(this, arguments);
}
function __PRIVATE_onWatchStreamClose2() {
	__PRIVATE_onWatchStreamClose2 = _asyncToGenerator(function* (e, t) {
...
function __PRIVATE_onWatchStreamChange(_x23, _x24, _x25) {
	return __PRIVATE_onWatchStreamChange2.apply(this, arguments);
}
function __PRIVATE_onWatchStreamChange2() {
	__PRIVATE_onWatchStreamChange2 = _asyncToGenerator(function* (e, t, n) {
		if (e.Aa.set("Online"), t instanceof __PRIVATE_WatchTargetChange && 2 === t.state && t.cause) try {
			/** Handles an error on a target */
			yield (/* @__PURE__ */ function() {
				var __PRIVATE_handleTargetError2 = _asyncToGenerator(function* (e, t) {
					const n = t.cause;
					for (const r of t.targetIds) e.Ta.has(r) && (yield e.remoteSyncer.rejectListen(r, n), e.Ta.delete(r), e.Ra.removeTarget(r));
				});
				function __PRIVATE_handleTargetError(_x21, _x22) {
					return __PRIVATE_handleTargetError2.apply(this, arguments);
				}
				return __PRIVATE_handleTargetError;
			}())(e, t);
		} catch (n) {
			__PRIVATE_logDebug(Xt, "Failed to remove targets %s: %s ", t.targetIds.join(","), n), yield __PRIVATE_disableNetworkUntilRecovery(e, n);
		}
			...
			n.compareTo(t) >= 0 && (yield function __PRIVATE_raiseWatchSnapshot(e, t) {
				const n = e.Ra.Pt(t);
			...
			__PRIVATE_logDebug(Xt, "Failed to raise snapshot:", t), yield __PRIVATE_disableNetworkUntilRecovery(e, t);
		}
	});
	return __PRIVATE_onWatchStreamChange2.apply(this, arguments);
}
...
* any retry attempt.
*/ function __PRIVATE_disableNetworkUntilRecovery(_x26, _x27, _x28) {
	return __PRIVATE_disableNetworkUntilRecovery2.apply(this, arguments);
}
function __PRIVATE_disableNetworkUntilRecovery2() {
	__PRIVATE_disableNetworkUntilRecovery2 = _asyncToGenerator(function* (e, t, n) {
		if (!__PRIVATE_isIndexedDbTransactionError(t)) throw t;
		e.Ia.add(1), yield __PRIVATE_disableNetworkInternal(e), e.Aa.set("Offline"), n || (n = () => __PRIVATE_localStoreGetLastRemoteSnapshotVersion(e.localStore)), e.asyncQueue.enqueueRetryable(_asyncToGenerator(function* () {
			__PRIVATE_logDebug(Xt, "Retrying IndexedDB access"), yield n(), e.Ia.delete(1), yield __PRIVATE_enableNetworkInternal(e);
		}));
	});
	return __PRIVATE_disableNetworkUntilRecovery2.apply(this, arguments);
}
...
function __PRIVATE_fillWritePipeline(_x29) {
	return __PRIVATE_fillWritePipeline2.apply(this, arguments);
}
function __PRIVATE_fillWritePipeline2() {
	__PRIVATE_fillWritePipeline2 = _asyncToGenerator(function* (e) {
		const t = __PRIVATE_debugCast(e), n = __PRIVATE_ensureWriteStream(t);
		let r = t.Pa.length > 0 ? t.Pa[t.Pa.length - 1].batchId : G;
		for (; __PRIVATE_canAddToWritePipeline(t);) try {
			const e = yield __PRIVATE_localStoreGetNextMutationBatch(t.localStore, r);
			if (null === e) {
				0 === t.Pa.length && n.B_();
				break;
			}
			...
			yield __PRIVATE_disableNetworkUntilRecovery(t, e);
		}
...
function __PRIVATE_onWriteStreamOpen(_x30) {
	return __PRIVATE_onWriteStreamOpen2.apply(this, arguments);
}
function __PRIVATE_onWriteStreamOpen2() {
	__PRIVATE_onWriteStreamOpen2 = _asyncToGenerator(function* (e) {
		__PRIVATE_ensureWriteStream(e).na();
	});
	return __PRIVATE_onWriteStreamOpen2.apply(this, arguments);
}
function __PRIVATE_onWriteHandshakeComplete(_x31) {
	return __PRIVATE_onWriteHandshakeComplete2.apply(this, arguments);
}
function __PRIVATE_onWriteHandshakeComplete2() {
	__PRIVATE_onWriteHandshakeComplete2 = _asyncToGenerator(function* (e) {
		const t = __PRIVATE_ensureWriteStream(e);
		for (const n of e.Pa) t.X_(n.mutations);
	});
	return __PRIVATE_onWriteHandshakeComplete2.apply(this, arguments);
}
function __PRIVATE_onMutationResult(_x32, _x33, _x34) {
	return __PRIVATE_onMutationResult2.apply(this, arguments);
}
function __PRIVATE_onMutationResult2() {
	__PRIVATE_onMutationResult2 = _asyncToGenerator(function* (e, t, n) {
		const r = e.Pa.shift(), i = MutationBatchResult.from(r, t, n);
		yield __PRIVATE_executeWithRecovery(e, (() => e.remoteSyncer.applySuccessfulWrite(i))), yield __PRIVATE_fillWritePipeline(e);
	});
	return __PRIVATE_onMutationResult2.apply(this, arguments);
}
function __PRIVATE_onWriteStreamClose(_x37, _x38) {
	return __PRIVATE_onWriteStreamClose2.apply(this, arguments);
}
function __PRIVATE_onWriteStreamClose2() {
	__PRIVATE_onWriteStreamClose2 = _asyncToGenerator(function* (e, t) {
		t && __PRIVATE_ensureWriteStream(e).Z_ && (yield (/* @__PURE__ */ function() {
			var __PRIVATE_handleWriteError2 = _asyncToGenerator(function* (e, t) {
				if (function __PRIVATE_isPermanentWriteError(e) {
					return __PRIVATE_isPermanentError(e) && e !== N.ABORTED;
				}(t.code)) {
					const n = e.Pa.shift();
					__PRIVATE_ensureWriteStream(e).N_(), yield __PRIVATE_executeWithRecovery(e, (() => e.remoteSyncer.rejectFailedWrite(n.batchId, t))), yield __PRIVATE_fillWritePipeline(e);
				}
			});
			function __PRIVATE_handleWriteError(_x35, _x36) {
				return __PRIVATE_handleWriteError2.apply(this, arguments);
			}
...
function __PRIVATE_remoteStoreHandleCredentialChange(_x39, _x40) {
	return __PRIVATE_remoteStoreHandleCredentialChange2.apply(this, arguments);
}
function __PRIVATE_remoteStoreHandleCredentialChange2() {
	__PRIVATE_remoteStoreHandleCredentialChange2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
		...
		n.Ia.add(3), yield __PRIVATE_disableNetworkInternal(n), r && n.Aa.set("Unknown"), yield n.remoteSyncer.handleCredentialChange(t), n.Ia.delete(3), yield __PRIVATE_enableNetworkInternal(n);
	});
	return __PRIVATE_remoteStoreHandleCredentialChange2.apply(this, arguments);
}
/**
* Toggles the network state when the client gains or loses its primary lease.
*/ function __PRIVATE_remoteStoreApplyPrimaryState(_x41, _x42) {
	return __PRIVATE_remoteStoreApplyPrimaryState2.apply(this, arguments);
}
function __PRIVATE_remoteStoreApplyPrimaryState2() {
	__PRIVATE_remoteStoreApplyPrimaryState2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
		t ? (n.Ia.delete(2), yield __PRIVATE_enableNetworkInternal(n)) : t || (n.Ia.add(2), yield __PRIVATE_disableNetworkInternal(n), n.Aa.set("Unknown"));
	});
	return __PRIVATE_remoteStoreApplyPrimaryState2.apply(this, arguments);
}
/**
...
*/ function __PRIVATE_ensureWatchStream(e) {
	return e.Va || (e.Va = function __PRIVATE_newPersistentWatchStream(e, t, n) {
		...
		var _ref2 = _asyncToGenerator(function* (t) {
			...
			* limitations under the License.
			*/
			t ? (e.Va.N_(), __PRIVATE_shouldStartWatchStream(e) ? __PRIVATE_startWatchStream(e) : e.Aa.set("Unknown")) : (yield e.Va.stop(), __PRIVATE_cleanUpWatchStreamState(e));
		});
...
*/ function __PRIVATE_ensureWriteStream(e) {
	return e.ma || (e.ma = function __PRIVATE_newPersistentWriteStream(e, t, n) {
		...
		var _ref3 = _asyncToGenerator(function* (t) {
			t ? (e.ma.N_(), yield __PRIVATE_fillWritePipeline(e)) : (yield e.ma.stop(), e.Pa.length > 0 && (__PRIVATE_logDebug(Xt, `Stopping write stream with ${e.Pa.length} pending writes`), e.Pa = []));
		});
...
function __PRIVATE_eventManagerListen(_x45, _x46) {
	return __PRIVATE_eventManagerListen2.apply(this, arguments);
}
function __PRIVATE_eventManagerListen2() {
	__PRIVATE_eventManagerListen2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
		let r = 3;
		const i = t.query;
...
function __PRIVATE_eventManagerUnlisten(_x47, _x48) {
	return __PRIVATE_eventManagerUnlisten2.apply(this, arguments);
}
function __PRIVATE_eventManagerUnlisten2() {
	__PRIVATE_eventManagerUnlisten2 = _asyncToGenerator(function* (e, t) {
...
*/ var __PRIVATE_BundleLoader = class {
	...
	*/ za(e) {
		var _this19 = this;
		return _asyncToGenerator(function* () {
			const t = yield __PRIVATE_localStoreApplyBundledDocuments(e, new __PRIVATE_BundleConverterImpl(_this19.serializer), _this19.Ka, _this19.$a.id), n = _this19.Ga(_this19.documents);
			for (const t of _this19.Ua) yield __PRIVATE_localStoreSaveNamedQuery(e, t, n.get(t.name));
			return _this19.progress.taskState = "Success", {
				progress: _this19.progress,
				ja: _this19.collectionGroups,
				Ja: t
			};
		})();
	}
...
*/
function __PRIVATE_syncEngineListen(_x49, _x50) {
	return __PRIVATE_syncEngineListen2.apply(this, arguments);
}
function __PRIVATE_syncEngineListen2() {
	__PRIVATE_syncEngineListen2 = _asyncToGenerator(function* (e, t, n = !0) {
		const r = __PRIVATE_ensureWatchCallbacks(e);
		let i;
		const s = r.Pu.get(t);
		return s ? (r.sharedClientState.addLocalQueryTarget(s.targetId), i = s.view.cu()) : i = yield __PRIVATE_allocateTargetAndMaybeListen(
			r,
			t,
			n,
...
/** Query has been listening to the cache, and tries to initiate the remote store listen */ function __PRIVATE_triggerRemoteStoreListen(_x51, _x52) {
	return __PRIVATE_triggerRemoteStoreListen2.apply(this, arguments);
}
function __PRIVATE_triggerRemoteStoreListen2() {
	__PRIVATE_triggerRemoteStoreListen2 = _asyncToGenerator(function* (e, t) {
...
function __PRIVATE_allocateTargetAndMaybeListen(_x53, _x54, _x55, _x56) {
	return __PRIVATE_allocateTargetAndMaybeListen2.apply(this, arguments);
}
function __PRIVATE_allocateTargetAndMaybeListen2() {
	__PRIVATE_allocateTargetAndMaybeListen2 = _asyncToGenerator(function* (e, t, n, r) {
...
* snapshot.
*/ function __PRIVATE_initializeViewAndComputeSnapshot(_x61, _x62, _x63, _x64, _x65) {
	return __PRIVATE_initializeViewAndComputeSnapshot2.apply(this, arguments);
}
function __PRIVATE_initializeViewAndComputeSnapshot2() {
	__PRIVATE_initializeViewAndComputeSnapshot2 = _asyncToGenerator(function* (e, t, n, r, i) {
		e.gu = (t, n, r) => (/* @__PURE__ */ function() {
			var __PRIVATE_applyDocChanges2 = _asyncToGenerator(function* (e, t, n, r) {
				let i = t.view.nu(n);
				i.Ds && (i = yield __PRIVATE_localStoreExecuteQuery(e.localStore, t.query, !1).then((({ documents: e }) => t.view.nu(e, i))));
			...
			function __PRIVATE_applyDocChanges(_x57, _x58, _x59, _x60) {
				return __PRIVATE_applyDocChanges2.apply(this, arguments);
			}
			return __PRIVATE_applyDocChanges;
		}())(e, t, n, r);
		const s = yield __PRIVATE_localStoreExecuteQuery(e.localStore, t, !0), o = new __PRIVATE_View(t, s.qs), _ = o.nu(s.documents), a = TargetChange.createSynthesizedTargetChangeForCurrentChange(n, r && "Offline" !== e.onlineState, i), u = o.applyChanges(_, e.isPrimaryClient, a);
...
/** Stops listening to the query. */ function __PRIVATE_syncEngineUnlisten(_x66, _x67, _x68) {
	return __PRIVATE_syncEngineUnlisten2.apply(this, arguments);
}
function __PRIVATE_syncEngineUnlisten2() {
	__PRIVATE_syncEngineUnlisten2 = _asyncToGenerator(function* (e, t, n) {
		...
		if (r.isPrimaryClient) {
			...
			r.sharedClientState.isActiveQueryTarget(i.targetId) || (yield __PRIVATE_localStoreReleaseTarget(r.localStore, i.targetId, !1).then((() => {
		...
		} else __PRIVATE_removeAndCleanupTarget(r, i.targetId), yield __PRIVATE_localStoreReleaseTarget(r.localStore, i.targetId, !0);
	});
	return __PRIVATE_syncEngineUnlisten2.apply(this, arguments);
}
/** Unlistens to the remote store while still listening to the cache. */ function __PRIVATE_triggerRemoteStoreUnlisten(_x69, _x70) {
	return __PRIVATE_triggerRemoteStoreUnlisten2.apply(this, arguments);
}
function __PRIVATE_triggerRemoteStoreUnlisten2() {
	__PRIVATE_triggerRemoteStoreUnlisten2 = _asyncToGenerator(function* (e, t) {
...
*/ function __PRIVATE_syncEngineWrite(_x71, _x72, _x73) {
	return __PRIVATE_syncEngineWrite2.apply(this, arguments);
}
function __PRIVATE_syncEngineWrite2() {
	__PRIVATE_syncEngineWrite2 = _asyncToGenerator(function* (e, t, n) {
		const r = __PRIVATE_syncEngineEnsureWriteCallbacks(e);
		try {
			const e = yield function __PRIVATE_localStoreWriteLocally(e, t) {
			...
			}(r, e.batchId, n), yield __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(r, e.changes), yield __PRIVATE_fillWritePipeline(r.remoteStore);
		} catch (e) {
...
*/ function __PRIVATE_syncEngineApplyRemoteEvent(_x74, _x75) {
	return __PRIVATE_syncEngineApplyRemoteEvent2.apply(this, arguments);
}
function __PRIVATE_syncEngineApplyRemoteEvent2() {
	__PRIVATE_syncEngineApplyRemoteEvent2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
		try {
			const e = yield __PRIVATE_localStoreApplyRemoteEventToLocalCache(n.localStore, t);
			...
			})), yield __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(n, e, t);
		} catch (e) {
			yield __PRIVATE_ignoreIfPrimaryLeaseLoss(e);
		}
	});
	return __PRIVATE_syncEngineApplyRemoteEvent2.apply(this, arguments);
}
...
*/ function __PRIVATE_syncEngineRejectListen(_x76, _x77, _x78) {
	return __PRIVATE_syncEngineRejectListen2.apply(this, arguments);
}
function __PRIVATE_syncEngineRejectListen2() {
	__PRIVATE_syncEngineRejectListen2 = _asyncToGenerator(function* (e, t, n) {
		const r = __PRIVATE_debugCast(e);
		...
		if (s) {
			...
			yield __PRIVATE_syncEngineApplyRemoteEvent(r, new RemoteEvent(SnapshotVersion.min(), /* @__PURE__ */ new Map(), new SortedMap(__PRIVATE_primitiveComparator), e, n)), r.du = r.du.remove(s), r.Eu.delete(t), __PRIVATE_pumpEnqueuedLimboResolutions(r);
		} else yield __PRIVATE_localStoreReleaseTarget(r.localStore, t, !1).then((() => __PRIVATE_removeAndCleanupTarget(r, t, n))).catch(__PRIVATE_ignoreIfPrimaryLeaseLoss);
	});
	return __PRIVATE_syncEngineRejectListen2.apply(this, arguments);
}
function __PRIVATE_syncEngineApplySuccessfulWrite(_x79, _x80) {
	return __PRIVATE_syncEngineApplySuccessfulWrite2.apply(this, arguments);
}
function __PRIVATE_syncEngineApplySuccessfulWrite2() {
	__PRIVATE_syncEngineApplySuccessfulWrite2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e), r = t.batch.batchId;
		try {
			const e = yield __PRIVATE_localStoreAcknowledgeBatch(n.localStore, t);
			__PRIVATE_processUserCallback(n, r, null), __PRIVATE_triggerPendingWritesCallbacks(n, r), n.sharedClientState.updateMutationState(r, "acknowledged"), yield __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(n, e);
		} catch (e) {
			yield __PRIVATE_ignoreIfPrimaryLeaseLoss(e);
		}
	});
...
function __PRIVATE_syncEngineRejectFailedWrite(_x81, _x82, _x83) {
	return __PRIVATE_syncEngineRejectFailedWrite2.apply(this, arguments);
}
function __PRIVATE_syncEngineRejectFailedWrite2() {
	__PRIVATE_syncEngineRejectFailedWrite2 = _asyncToGenerator(function* (e, t, n) {
		const r = __PRIVATE_debugCast(e);
		try {
			const e = yield function __PRIVATE_localStoreRejectBatch(e, t) {
				const n = __PRIVATE_debugCast(e);
			...
			__PRIVATE_processUserCallback(r, t, n), __PRIVATE_triggerPendingWritesCallbacks(r, t), r.sharedClientState.updateMutationState(t, "rejected", n), yield __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(r, e);
		} catch (n) {
			yield __PRIVATE_ignoreIfPrimaryLeaseLoss(n);
		}
	});
	return __PRIVATE_syncEngineRejectFailedWrite2.apply(this, arguments);
}
...
* are acknowledged .
*/ function __PRIVATE_syncEngineRegisterPendingWritesCallback(_x84, _x85) {
	return __PRIVATE_syncEngineRegisterPendingWritesCallback2.apply(this, arguments);
}
function __PRIVATE_syncEngineRegisterPendingWritesCallback2() {
	__PRIVATE_syncEngineRegisterPendingWritesCallback2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
		...
		try {
			const e = yield function __PRIVATE_localStoreGetHighestUnacknowledgedBatchId(e) {
				const t = __PRIVATE_debugCast(e);
...
function __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(_x88, _x89, _x90) {
	return __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore2.apply(this, arguments);
}
function __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore2() {
	__PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore2 = _asyncToGenerator(function* (e, t, n) {
		...
		r.Pu.isEmpty() || (r.Pu.forEach(((e, _) => {
		...
		})), yield Promise.all(o), r.hu.J_(i), yield (/* @__PURE__ */ function() {
			var __PRIVATE_localStoreNotifyLocalViewChanges2 = _asyncToGenerator(function* (e, t) {
				const n = __PRIVATE_debugCast(e);
				try {
					yield n.persistence.runTransaction("notifyLocalViewChanges", "readwrite", ((e) => PersistencePromise.forEach(t, ((t) => PersistencePromise.forEach(t.Is, ((r) => n.persistence.referenceDelegate.addReference(e, t.targetId, r))).next((() => PersistencePromise.forEach(t.ds, ((r) => n.persistence.referenceDelegate.removeReference(e, t.targetId, r)))))))));
				} catch (e) {
					if (!__PRIVATE_isIndexedDbTransactionError(e)) throw e;
			...
			function __PRIVATE_localStoreNotifyLocalViewChanges(_x86, _x87) {
				return __PRIVATE_localStoreNotifyLocalViewChanges2.apply(this, arguments);
			}
...
function __PRIVATE_syncEngineHandleCredentialChange(_x91, _x92) {
	return __PRIVATE_syncEngineHandleCredentialChange2.apply(this, arguments);
}
function __PRIVATE_syncEngineHandleCredentialChange2() {
	__PRIVATE_syncEngineHandleCredentialChange2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
		if (!n.currentUser.isEqual(t)) {
			__PRIVATE_logDebug(nn, "User change. New user:", t.toKey());
			const e = yield __PRIVATE_localStoreHandleUserChange(n.localStore, t);
			...
			}(n, "'waitForPendingWrites' promise is rejected due to a user change."), n.sharedClientState.handleUserChange(t, e.removedBatchIds, e.addedBatchIds), yield __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(n, e.Bs);
		}
	});
	return __PRIVATE_syncEngineHandleCredentialChange2.apply(this, arguments);
}
...
* from persistence.
*/ function __PRIVATE_synchronizeViewAndComputeSnapshot(_x93, _x94) {
	return __PRIVATE_synchronizeViewAndComputeSnapshot2.apply(this, arguments);
}
function __PRIVATE_synchronizeViewAndComputeSnapshot2() {
	__PRIVATE_synchronizeViewAndComputeSnapshot2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e), r = yield __PRIVATE_localStoreExecuteQuery(n.localStore, t.query, !0), i = t.view.uu(r);
...
* snapshots if needed.
*/
function __PRIVATE_syncEngineSynchronizeWithChangedDocuments(_x95, _x96) {
	return __PRIVATE_syncEngineSynchronizeWithChangedDocuments2.apply(this, arguments);
}
function __PRIVATE_syncEngineSynchronizeWithChangedDocuments2() {
	__PRIVATE_syncEngineSynchronizeWithChangedDocuments2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
...
function __PRIVATE_syncEngineApplyBatchState(_x97, _x98, _x99, _x100) {
	return __PRIVATE_syncEngineApplyBatchState2.apply(this, arguments);
}
function __PRIVATE_syncEngineApplyBatchState2() {
	__PRIVATE_syncEngineApplyBatchState2 = _asyncToGenerator(function* (e, t, n, r) {
		const i = __PRIVATE_debugCast(e), s = yield function __PRIVATE_localStoreLookupMutationDocuments(e, t) {
		...
		null !== s ? ("pending" === n ? yield __PRIVATE_fillWritePipeline(i.remoteStore) : "acknowledged" === n || "rejected" === n ? (__PRIVATE_processUserCallback(i, t, r || null), __PRIVATE_triggerPendingWritesCallbacks(i, t), function __PRIVATE_localStoreRemoveCachedMutationBatchMetadata(e, t) {
			__PRIVATE_debugCast(__PRIVATE_debugCast(e).mutationQueue).rr(t);
		}(i.localStore, t)) : fail(6720, "Unknown batchState", { wu: n }), yield __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(i, s)) : __PRIVATE_logDebug(nn, "Cannot apply mutation batch with id: " + t);
	});
	return __PRIVATE_syncEngineApplyBatchState2.apply(this, arguments);
}
/** Applies a query target change from a different tab. */
function __PRIVATE_syncEngineApplyPrimaryState(_x101, _x102) {
	return __PRIVATE_syncEngineApplyPrimaryState2.apply(this, arguments);
}
function __PRIVATE_syncEngineApplyPrimaryState2() {
	__PRIVATE_syncEngineApplyPrimaryState2 = _asyncToGenerator(function* (e, t) {
		const n = __PRIVATE_debugCast(e);
		if (__PRIVATE_ensureWatchCallbacks(n), __PRIVATE_syncEngineEnsureWriteCallbacks(n), !0 === t && !0 !== n.fu) {
			const t = yield __PRIVATE_synchronizeQueryViewsAndRaiseSnapshots(n, n.sharedClientState.getAllActiveQueryTargets().toArray());
			n.fu = !0, yield __PRIVATE_remoteStoreApplyPrimaryState(n.remoteStore, !0);
			...
			})), yield t, yield __PRIVATE_synchronizeQueryViewsAndRaiseSnapshots(n, e), function __PRIVATE_resetLimboDocuments(e) {
				const t = __PRIVATE_debugCast(e);
				t.Eu.forEach(((e, n) => {
					__PRIVATE_remoteStoreUnlisten(t.remoteStore, n);
				})), t.Au.jr(), t.Eu = /* @__PURE__ */ new Map(), t.du = new SortedMap(DocumentKey.comparator);
			}(n), n.fu = !1, yield __PRIVATE_remoteStoreApplyPrimaryState(n.remoteStore, !1);
		}
	});
	return __PRIVATE_syncEngineApplyPrimaryState2.apply(this, arguments);
}
function __PRIVATE_synchronizeQueryViewsAndRaiseSnapshots(_x103, _x104, _x105) {
	return __PRIVATE_synchronizeQueryViewsAndRaiseSnapshots2.apply(this, arguments);
}
function __PRIVATE_synchronizeQueryViewsAndRaiseSnapshots2() {
	__PRIVATE_synchronizeQueryViewsAndRaiseSnapshots2 = _asyncToGenerator(function* (e, t, n) {
		const r = __PRIVATE_debugCast(e), i = [], s = [];
		for (const e of t) {
			let t;
			const n = r.Tu.get(e);
			if (n && 0 !== n.length) {
				t = yield __PRIVATE_localStoreAllocateTarget(r.localStore, __PRIVATE_queryToTarget(n[0]));
				for (const e of n) {
					const n = yield __PRIVATE_synchronizeViewAndComputeSnapshot(r, r.Pu.get(e));
					n.snapshot && s.push(n.snapshot);
				}
			} else {
				const n = yield __PRIVATE_localStoreGetCachedTarget(r.localStore, e);
				t = yield __PRIVATE_localStoreAllocateTarget(r.localStore, n), yield __PRIVATE_initializeViewAndComputeSnapshot(r, __PRIVATE_synthesizeTargetToQuery(n), e, !1, t.resumeToken);
			}
			i.push(t);
		}
		return r.hu.J_(s), i;
...
/** Applies a query target change from a different tab. */
function __PRIVATE_syncEngineApplyTargetState(_x106, _x107, _x108, _x109) {
	return __PRIVATE_syncEngineApplyTargetState2.apply(this, arguments);
}
function __PRIVATE_syncEngineApplyTargetState2() {
	__PRIVATE_syncEngineApplyTargetState2 = _asyncToGenerator(function* (e, t, n, r) {
		const i = __PRIVATE_debugCast(e);
		...
		if (s && s.length > 0) switch (n) {
			case "current":
			case "not-current":
				yield __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(i, yield __PRIVATE_localStoreGetNewDocumentChanges(i.localStore, __PRIVATE_queryCollectionGroup(s[0])), RemoteEvent.createSynthesizedRemoteEventForCurrentChange(t, "current" === n, ByteString.EMPTY_BYTE_STRING));
				break;
			case "rejected":
				yield __PRIVATE_localStoreReleaseTarget(i.localStore, t, !0), __PRIVATE_removeAndCleanupTarget(i, t, r);
				break;
...
/** Adds or removes Watch targets for queries from different tabs. */ function __PRIVATE_syncEngineApplyActiveTargetsChange(_x110, _x111, _x112) {
	return __PRIVATE_syncEngineApplyActiveTargetsChange2.apply(this, arguments);
}
function __PRIVATE_syncEngineApplyActiveTargetsChange2() {
	__PRIVATE_syncEngineApplyActiveTargetsChange2 = _asyncToGenerator(function* (e, t, n) {
		const r = __PRIVATE_ensureWatchCallbacks(e);
		if (r.fu) {
			for (const e of t) {
				...
				const t = yield __PRIVATE_localStoreGetCachedTarget(r.localStore, e), n = yield __PRIVATE_localStoreAllocateTarget(r.localStore, t);
				yield __PRIVATE_initializeViewAndComputeSnapshot(r, __PRIVATE_synthesizeTargetToQuery(t), n.targetId, !1, n.resumeToken), __PRIVATE_remoteStoreListen(r.remoteStore, n);
			}
			for (const e of n) r.Tu.has(e) && (yield __PRIVATE_localStoreReleaseTarget(r.localStore, e, !1).then((() => {
...
function() {
		var __PRIVATE_loadBundleImpl2 = _asyncToGenerator(function* (e, t, n) {
			try {
				const r = yield t.getMetadata();
				if (yield function __PRIVATE_localStoreHasNewerBundle(e, t) {
				...
				}(e.localStore, r)) return yield t.close(), n._completeWith(function __PRIVATE_bundleSuccessProgress(e) {
					return {
						taskState: "Success",
				...
				let s = yield t.Su();
				for (; s;) {
					const e = yield i.Wa(s);
					e && n._updateProgress(e), s = yield t.Su();
				}
				const o = yield i.za(e.localStore);
				return yield __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(e, o.Ja, void 0), yield function __PRIVATE_localStoreSaveBundle(e, t) {
					const n = __PRIVATE_debugCast(e);
		...
		function __PRIVATE_loadBundleImpl(_x113, _x114, _x115) {
			return __PRIVATE_loadBundleImpl2.apply(this, arguments);
		}
...
var __PRIVATE_MemoryOfflineComponentProvider = class {
	...
	initialize(e) {
		var _this20 = this;
		return _asyncToGenerator(function* () {
			_this20.serializer = __PRIVATE_newSerializer(e.databaseInfo.databaseId), _this20.sharedClientState = _this20.bu(e), _this20.persistence = _this20.Du(e), yield _this20.persistence.start(), _this20.localStore = _this20.vu(e), _this20.gcScheduler = _this20.Cu(e, _this20.localStore), _this20.indexBackfillerScheduler = _this20.Fu(e, _this20.localStore);
		})();
	}
	Cu(e, t) {
		return null;
...
*/ var __PRIVATE_IndexedDbOfflineComponentProvider = class extends __PRIVATE_MemoryOfflineComponentProvider {
	...
	initialize(e) {
		var _superprop_getInitialize = () => super.initialize, _this22 = this;
		return _asyncToGenerator(function* () {
			yield _superprop_getInitialize().call(_this22, e), yield _this22.Mu.initialize(_this22, e), yield __PRIVATE_syncEngineEnsureWriteCallbacks(_this22.Mu.syncEngine), yield __PRIVATE_fillWritePipeline(_this22.Mu.remoteStore), yield _this22.persistence.ji((() => (_this22.gcScheduler && !_this22.gcScheduler.started && _this22.gcScheduler.start(), _this22.indexBackfillerScheduler && !_this22.indexBackfillerScheduler.started && _this22.indexBackfillerScheduler.start(), Promise.resolve())));
		})();
	}
	vu(e) {
		return __PRIVATE_newLocalStore(this.persistence, new __PRIVATE_QueryEngine(), e.initialUser, this.serializer);
...
*/ var __PRIVATE_MultiTabOfflineComponentProvider = class extends __PRIVATE_IndexedDbOfflineComponentProvider {
	...
	initialize(e) {
		var _superprop_getInitialize2 = () => super.initialize, _this23 = this;
		return _asyncToGenerator(function* () {
			yield _superprop_getInitialize2().call(_this23, e);
			const t = _this23.Mu.syncEngine;
			...
			}, yield _this23.sharedClientState.start()), yield _this23.persistence.ji(function() {
				var _ref4 = _asyncToGenerator(function* (e) {
					yield __PRIVATE_syncEngineApplyPrimaryState(_this23.Mu.syncEngine, e), _this23.gcScheduler && (e && !_this23.gcScheduler.started ? _this23.gcScheduler.start() : e || _this23.gcScheduler.stop()), _this23.indexBackfillerScheduler && (e && !_this23.indexBackfillerScheduler.started ? _this23.indexBackfillerScheduler.start() : e || _this23.indexBackfillerScheduler.stop());
				});
...
*/ var OnlineComponentProvider = class {
	initialize(e, t) {
		var _this24 = this;
		return _asyncToGenerator(function* () {
			_this24.localStore || (_this24.localStore = e.localStore, _this24.sharedClientState = e.sharedClientState, _this24.datastore = _this24.createDatastore(t), _this24.remoteStore = _this24.createRemoteStore(t), _this24.eventManager = _this24.createEventManager(t), _this24.syncEngine = _this24.createSyncEngine(t, !e.synchronizeTabs), _this24.sharedClientState.onlineStateHandler = (e) => __PRIVATE_syncEngineApplyOnlineStateChange(_this24.syncEngine, e, 1), _this24.remoteStore.remoteSyncer.handleCredentialChange = __PRIVATE_syncEngineHandleCredentialChange.bind(null, _this24.syncEngine), yield __PRIVATE_remoteStoreApplyPrimaryState(_this24.remoteStore, _this24.syncEngine.isPrimaryClient));
		})();
	}
	...
	terminate() {
		...
		return _asyncToGenerator(function* () {
			var e, t;
			yield (/* @__PURE__ */ function() {
				var __PRIVATE_remoteStoreShutdown2 = _asyncToGenerator(function* (e) {
					const t = __PRIVATE_debugCast(e);
					__PRIVATE_logDebug(Xt, "RemoteStore shutting down."), t.Ia.add(5), yield __PRIVATE_disableNetworkInternal(t), t.Ea.shutdown(), t.Aa.set("Unknown");
				});
				function __PRIVATE_remoteStoreShutdown(_x117) {
					return __PRIVATE_remoteStoreShutdown2.apply(this, arguments);
				}
...
*/ var __PRIVATE_BundleReaderImpl = class {
	constructor(e, t) {
		this.Nu = e, this.serializer = t, this.metadata = new __PRIVATE_Deferred(), this.buffer = /* @__PURE__ */ new Uint8Array(), this.Bu = function __PRIVATE_newTextDecoder() {
	...
	Su() {
		var _this27 = this;
		return _asyncToGenerator(function* () {
			return yield _this27.getMetadata(), _this27.Lu();
		})();
	}
	/**
	...
	*/ Lu() {
		var _this28 = this;
		return _asyncToGenerator(function* () {
			const e = yield _this28.ku();
			if (null === e) return null;
			const t = _this28.Bu.decode(e), n = Number(t);
			isNaN(n) && _this28.qu(`length string (${t}) is not valid number`);
			const r = yield _this28.Qu(n);
	...
	*/ ku() {
		var _this29 = this;
		return _asyncToGenerator(function* () {
			for (; _this29.$u() < 0;) if (yield _this29.Uu()) break;
			if (0 === _this29.buffer.length) return null;
			const e = _this29.$u();
			e < 0 && _this29.qu("Reached the end of bundle when a length string is expected.");
	...
	*/ Qu(e) {
		var _this30 = this;
		return _asyncToGenerator(function* () {
			for (; _this30.buffer.length < e;) (yield _this30.Uu()) && _this30.qu("Reached the end of bundle when more is expected.");
			const t = _this30.Bu.decode(_this30.buffer.slice(0, e));
...
var Transaction$2 = class {
	...
	lookup(e) {
		var _this32 = this;
		return _asyncToGenerator(function* () {
			if (_this32.ensureCommitNotCalled(), _this32.mutations.length > 0) throw _this32.lastTransactionError = new FirestoreError(N.INVALID_ARGUMENT, "Firestore transactions require all reads to be executed before all writes."), _this32.lastTransactionError;
			const t = yield (/* @__PURE__ */ function() {
				var __PRIVATE_invokeBatchGetDocumentsRpc2 = _asyncToGenerator(function* (e, t) {
					const n = __PRIVATE_debugCast(e), r = { documents: t.map(((e) => __PRIVATE_toName(n.serializer, e))) }, i = yield n.Jo("BatchGetDocuments", n.serializer.databaseId, ResourcePath.emptyPath(), r, t.length), s = /* @__PURE__ */ new Map();
					i.forEach(((e) => {
				...
				function __PRIVATE_invokeBatchGetDocumentsRpc(_x118, _x119) {
					return __PRIVATE_invokeBatchGetDocumentsRpc2.apply(this, arguments);
				}
	...
	commit() {
		...
		return _asyncToGenerator(function* () {
			...
			_this33.mutations.forEach(((t) => {
				...
				var __PRIVATE_invokeCommitRpc2 = _asyncToGenerator(function* (e, t) {
				...
				function __PRIVATE_invokeCommitRpc(_x120, _x121) {
					return __PRIVATE_invokeCommitRpc2.apply(this, arguments);
				}
...
*/ var __PRIVATE_TransactionRunner = class {
	...
	ju() {
		var _this34 = this;
		this.F_.g_(_asyncToGenerator(function* () {
			const e = new Transaction$2(_this34.datastore), t = _this34.Ju(e);
			t && t.then(((t) => {
...
var FirestoreClient = class {
	constructor(e, t, n, r, i) {
		var _this35 = this;
		this.authCredentials = e, this.appCheckCredentials = t, this.asyncQueue = n, this.databaseInfo = r, this.user = User.UNAUTHENTICATED, this.clientId = __PRIVATE_AutoId.newId(), this.authCredentialListener = () => Promise.resolve(), this.appCheckCredentialListener = () => Promise.resolve(), this._uninitializedComponentsProvider = i, this.authCredentials.start(n, function() {
			var _ref5 = _asyncToGenerator(function* (e) {
				__PRIVATE_logDebug(rn, "Received user=", e.uid), yield _this35.authCredentialListener(e), _this35.user = e;
			});
	...
	terminate() {
		...
		return this.asyncQueue.enqueueAndForgetEvenWhileRestricted(_asyncToGenerator(function* () {
			try {
				_this36._onlineComponents && (yield _this36._onlineComponents.terminate()), _this36._offlineComponents && (yield _this36._offlineComponents.terminate()), _this36.authCredentials.shutdown(), _this36.appCheckCredentials.shutdown(), e.resolve();
			} catch (t) {
...
function __PRIVATE_setOfflineComponentProvider(_x124, _x125) {
	return __PRIVATE_setOfflineComponentProvider2.apply(this, arguments);
}
function __PRIVATE_setOfflineComponentProvider2() {
	__PRIVATE_setOfflineComponentProvider2 = _asyncToGenerator(function* (e, t) {
		...
		let r = n.initialUser;
		e.setCredentialChangeListener(function() {
...
function __PRIVATE_setOnlineComponentProvider(_x126, _x127) {
	return __PRIVATE_setOnlineComponentProvider2.apply(this, arguments);
}
function __PRIVATE_setOnlineComponentProvider2() {
	__PRIVATE_setOnlineComponentProvider2 = _asyncToGenerator(function* (e, t) {
		e.asyncQueue.verifyOperationInProgress();
		const n = yield __PRIVATE_ensureOfflineComponents(e);
		__PRIVATE_logDebug(rn, "Initializing OnlineComponentProvider"), yield t.initialize(n, e.configuration), e.setCredentialChangeListener(((e) => __PRIVATE_remoteStoreHandleCredentialChange(t.remoteStore, e))), e.setAppCheckTokenChangeListener(((e, n) => __PRIVATE_remoteStoreHandleCredentialChange(t.remoteStore, n))), e._onlineComponents = t;
	});
	return __PRIVATE_setOnlineComponentProvider2.apply(this, arguments);
}
/**
...
*/ function __PRIVATE_ensureOfflineComponents(_x128) {
	return __PRIVATE_ensureOfflineComponents2.apply(this, arguments);
}
function __PRIVATE_ensureOfflineComponents2() {
	__PRIVATE_ensureOfflineComponents2 = _asyncToGenerator(function* (e) {
...
function __PRIVATE_ensureOnlineComponents(_x129) {
	return __PRIVATE_ensureOnlineComponents2.apply(this, arguments);
}
function __PRIVATE_ensureOnlineComponents2() {
	__PRIVATE_ensureOnlineComponents2 = _asyncToGenerator(function* (e) {
...
function __PRIVATE_getEventManager(_x130) {
	return __PRIVATE_getEventManager2.apply(this, arguments);
}
function __PRIVATE_getEventManager2() {
	__PRIVATE_getEventManager2 = _asyncToGenerator(function* (e) {
...
/** Enables the network connection and re-enqueues all pending operations. */ function __PRIVATE_firestoreClientEnableNetwork(e) {
	return e.asyncQueue.enqueue(_asyncToGenerator(function* () {
...
/** Disables the network connection. Pending operations will not complete. */ function __PRIVATE_firestoreClientDisableNetwork(e) {
	return e.asyncQueue.enqueue(_asyncToGenerator(function* () {
		const t = yield __PRIVATE_getPersistence(e), n = yield __PRIVATE_getRemoteStore(e);
		return t.setNetworkEnabled(!1), (/* @__PURE__ */ function() {
			var __PRIVATE_remoteStoreDisableNetwork2 = _asyncToGenerator(function* (e) {
				const t = __PRIVATE_debugCast(e);
				t.Ia.add(0), yield __PRIVATE_disableNetworkInternal(t), t.Aa.set("Offline");
			});
			function __PRIVATE_remoteStoreDisableNetwork(_x131) {
				return __PRIVATE_remoteStoreDisableNetwork2.apply(this, arguments);
			}
...
*/ function __PRIVATE_firestoreClientGetDocumentFromLocalCache(e, t) {
	...
	return e.asyncQueue.enqueueAndForget(_asyncToGenerator(function* () {
		return (/* @__PURE__ */ function() {
			var __PRIVATE_readDocumentFromCache2 = _asyncToGenerator(function* (e, t, n) {
				try {
					const r = yield function __PRIVATE_localStoreReadDocument(e, t) {
						const n = __PRIVATE_debugCast(e);
			...
			function __PRIVATE_readDocumentFromCache(_x132, _x133, _x134) {
				return __PRIVATE_readDocumentFromCache2.apply(this, arguments);
			}
...
function __PRIVATE_firestoreClientGetDocumentsFromLocalCache(e, t) {
	...
	return e.asyncQueue.enqueueAndForget(_asyncToGenerator(function* () {
		return (/* @__PURE__ */ function() {
			var __PRIVATE_executeQueryFromCache2 = _asyncToGenerator(function* (e, t, n) {
				try {
					const r = yield __PRIVATE_localStoreExecuteQuery(e, t, !0), i = new __PRIVATE_View(t, r.qs), s = i.nu(r.documents), o = i.applyChanges(s, !1);
			...
			function __PRIVATE_executeQueryFromCache(_x135, _x136, _x137) {
				return __PRIVATE_executeQueryFromCache2.apply(this, arguments);
			}
...
function __PRIVATE_firestoreClientRunAggregateQuery(e, t, n) {
	...
	return e.asyncQueue.enqueueAndForget(_asyncToGenerator(function* () {
		try {
			...
			r.resolve((/* @__PURE__ */ function() {
				var __PRIVATE_invokeRunAggregationQueryRpc2 = _asyncToGenerator(function* (e, t, n) {
					var r;
					...
					const a = (yield i.Jo("RunAggregationQuery", i.serializer.databaseId, _, s, 1)).filter(((e) => !!e.result));
				...
				function __PRIVATE_invokeRunAggregationQueryRpc(_x138, _x139, _x140) {
					return __PRIVATE_invokeRunAggregationQueryRpc2.apply(this, arguments);
				}
...
function __PRIVATE_firestoreClientAddSnapshotsInSyncListener(e, t) {
	...
	return e.asyncQueue.enqueueAndForget(_asyncToGenerator(function* () {
		...
		n.Ou(), e.asyncQueue.enqueueAndForget(_asyncToGenerator(function* () {
...
*/ function __PRIVATE_firestoreClientLoadBundle(e, t, n, r) {
	...
	e.asyncQueue.enqueueAndForget(_asyncToGenerator(function* () {
...
function __PRIVATE_firestoreClientSetIndexConfiguration(e, t) {
	return e.asyncQueue.enqueue(_asyncToGenerator(function* () {
		return (/* @__PURE__ */ function() {
			var __PRIVATE_localStoreConfigureFieldIndexes2 = _asyncToGenerator(function* (e, t) {
			...
			function __PRIVATE_localStoreConfigureFieldIndexes(_x141, _x142) {
				return __PRIVATE_localStoreConfigureFieldIndexes2.apply(this, arguments);
			}
...
* Security Rules.
*/ function connectFirestoreEmulator$1(e, t, n, r = {}) {
	var i;
...
*/ function collectionGroup$1(e, t) {
...
var __PRIVATE_AsyncQueueImpl = class {
	...
	*/ cc() {
		var _this38 = this;
		return _asyncToGenerator(function* () {
			if (0 !== _this38.Zu.length) {
				try {
					yield _this38.Zu[0](), _this38.Zu.shift(), _this38.F_.reset();
				} catch (e) {
					if (!__PRIVATE_isIndexedDbTransactionError(e)) throw e;
					__PRIVATE_logDebug(an, "Operation failed with retryable error: " + e);
				}
				_this38.Zu.length > 0 && _this38.F_.g_((() => _this38.cc()));
			}
		})();
	}
	uc(e) {
...
*/ var Firestore$2 = class extends Firestore$1 {
	/** @hideconstructor */
	constructor(e, t, n, r) {
...
*/ function initializeFirestore$1(e, t, n) {
	n || (n = ut);
...
function getFirestore$1(e, n) {
	...
	if (!s._initialized) {
		...
		e && connectFirestoreEmulator$1(s, ...e);
	}
	return s;
}
/**
...
function enableIndexedDbPersistence$1(e, t) {
...
*/ function enableMultiTabIndexedDbPersistence$1(_x143) {
	return _enableMultiTabIndexedDbPersistence.apply(this, arguments);
}
function _enableMultiTabIndexedDbPersistence() {
	_enableMultiTabIndexedDbPersistence = _asyncToGenerator(function* (e) {
...
*/ function __PRIVATE_setPersistenceProviders(e, t, n) {
	if ((e = __PRIVATE_cast(e, Firestore$2))._firestoreClient || e._terminated) throw new FirestoreError(N.FAILED_PRECONDITION, "Firestore has already been started and persistence can no longer be enabled. You can only enable persistence before calling any other methods on a Firestore object.");
...
*/ function clearIndexedDbPersistence$1(e) {
	...
	const t = new __PRIVATE_Deferred();
	return e._queue.enqueueAndForgetEvenWhileRestricted(_asyncToGenerator(function* () {
		try {
			yield (/* @__PURE__ */ function() {
				var __PRIVATE_indexedDbClearPersistence2 = _asyncToGenerator(function* (e) {
					if (!__PRIVATE_SimpleDb.C()) return Promise.resolve();
					const t = e + qt;
					yield __PRIVATE_SimpleDb.delete(t);
				});
				function __PRIVATE_indexedDbClearPersistence(_x144) {
					return __PRIVATE_indexedDbClearPersistence2.apply(this, arguments);
				}
...
* acknowledged by the backend.
*/ function waitForPendingWrites$1(e) {
	...
	}(ensureFirestoreConfigured(e = __PRIVATE_cast(e, Firestore$2)));
}
/**
...
*/ function enableNetwork$1(e) {
	return __PRIVATE_firestoreClientEnableNetwork(ensureFirestoreConfigured(e = __PRIVATE_cast(e, Firestore$2)));
}
/**
...
*/ function disableNetwork$1(e) {
	return __PRIVATE_firestoreClientDisableNetwork(ensureFirestoreConfigured(e = __PRIVATE_cast(e, Firestore$2)));
}
/**
...
*/ function loadBundle$1(e, t) {
	const n = ensureFirestoreConfigured(e = __PRIVATE_cast(e, Firestore$2)), r = new LoadBundleTask();
...
*/ function namedQuery$1(e, t) {
	return __PRIVATE_firestoreClientGetNamedQuery(ensureFirestoreConfigured(e = __PRIVATE_cast(e, Firestore$2)), t).then(((t) => t ? new Query(e, null, t.query) : null));
}
/**
* @license
* Copyright 2020 Google LLC
...
*/ function aggregateFieldEqual$1(e, t) {
	var n, r;
...
* otherwise.
*/ function aggregateQuerySnapshotEqual$1(e, t) {
	return queryEqual$1(e.query, t.query) && deepEqual(e.data(), t.data());
}
/**
* @license
* Copyright 2025 Google LLC
...
*
* @returns A `Promise` that will be resolved with the results of the query.
*/ function getDocsFromServer$1(e) {
	e = __PRIVATE_cast(e, Query);
	const t = __PRIVATE_cast(e.firestore, Firestore$2), n = ensureFirestoreConfigured(t), r = new __PRIVATE_ExpUserDataWriter(t);
...
*/ function addDoc$1(e, t) {
	const n = __PRIVATE_cast(e.firestore, Firestore$2), r = doc$2(e), i = __PRIVATE_applyFirestoreDataConverter(e.converter, t);
...
function onSnapshot$1(e, ...t) {
	...
	let a, u, c;
	if (e instanceof DocumentReference) u = __PRIVATE_cast(e.firestore, Firestore$2), c = __PRIVATE_newQueryForPath(e._key.path), a = {
		next: (n) => {
		...
		u = __PRIVATE_cast(n.firestore, Firestore$2), c = n._query;
	...
	return function __PRIVATE_firestoreClientListen(e, t, n, r) {
		...
		return e.asyncQueue.enqueueAndForget(_asyncToGenerator(function* () {
			...
			i.Ou(), e.asyncQueue.enqueueAndForget(_asyncToGenerator(function* () {
...
function onSnapshotResume(e, t, ...n) {
	...
	if ("object" != typeof n[o] || __PRIVATE_isPartialObserver(n[o]) || (s = n[o++]), "QuerySnapshot" === i.bundleSource) {
		...
		*/
		return function __PRIVATE_onSnapshotQuerySnapshotBundle(e, t, n, r, i) {
			let s, o = !1;
			return loadBundle$1(e, t.bundle).then((() => namedQuery$1(e, t.bundleName))).then(((e) => {
				if (e && !o) i && e.withConverter(i), s = onSnapshot$1(e, n || {}, r);
	...
	if ("DocumentSnapshot" === i.bundleSource) {
		...
		return function __PRIVATE_onSnapshotDocumentSnapshotBundle(e, t, n, r, i) {
			let s, o = !1;
			return loadBundle$1(e, t.bundle).then((() => {
				if (!o) s = onSnapshot$1(new DocumentReference(e, i || null, DocumentKey.fromPath(t.bundleName)), n || {}, r);
			})).catch(((e) => (r.error && r.error(e), () => {}))), () => {
				o || (o = !0, s && s());
			};
		}(r, i, s, e, n[o]);
...
function onSnapshotsInSync$1(e, t) {
	return __PRIVATE_firestoreClientAddSnapshotsInSyncListener(ensureFirestoreConfigured(e = __PRIVATE_cast(e, Firestore$2)), __PRIVATE_isPartialObserver(t) ? t : { next: t });
}
/**
...
function getCountFromServer$1(e) {
	return getAggregateFromServer$1(e, { count: count() });
}
/**
...
* ```
*/ function getAggregateFromServer$1(e, t) {
	const n = __PRIVATE_cast(e.firestore, Firestore$2), r = ensureFirestoreConfigured(n), i = __PRIVATE_mapToArray(t, ((e, t) => new __PRIVATE_AggregateImpl(t, e.aggregateType, e._internalFieldPath)));
...
*/ function runTransaction$1(e, t, n) {
	e = __PRIVATE_cast(e, Firestore$2);
...
* writes.
*/ function writeBatch$1(e) {
	return ensureFirestoreConfigured(e = __PRIVATE_cast(e, Firestore$2)), new WriteBatch(e, ((t) => executeWrite(e, t)));
}
/**
* @license
* Copyright 2021 Google LLC
...
* See the License for the specific language governing permissions and
* limitations under the License.
*/ function setIndexConfiguration$1(e, t) {
	const n = ensureFirestoreConfigured(e = __PRIVATE_cast(e, Firestore$2));
...
*/ function getPersistentCacheIndexManager$1(e) {
	var t;
	e = __PRIVATE_cast(e, Firestore$2);
	const n = Tn.get(e);
	if (n) return n;
...
*/ function enablePersistentCacheIndexAutoCreation$1(e) {
...
*/ function disablePersistentCacheIndexAutoCreation$1(e) {
...
*/ function deleteAllPersistentCacheIndexes$1(e) {
...
function _internalQueryToProtoQueryTarget(e) {
	var t;
	const n = null === (t = ensureFirestoreConfigured(__PRIVATE_cast(e.firestore, Firestore$2))._onlineComponents) || void 0 === t ? void 0 : t.datastore.serializer;
...
*/ function _internalAggregationQueryToProtoRunAggregationQueryRequest(e, t) {
	var n;
	const r = __PRIVATE_mapToArray(t, ((e, t) => new __PRIVATE_AggregateImpl(t, e.aggregateType, e._internalFieldPath))), i = null === (n = ensureFirestoreConfigured(__PRIVATE_cast(e.firestore, Firestore$2))._onlineComponents) || void 0 === n ? void 0 : n.datastore.serializer;
...
*/ (function __PRIVATE_registerFirestore(e, t = !0) {
	(function __PRIVATE_setSDKVersion(e) {
		...
		const i = e.getProvider("app").getImmediate(), s = new Firestore$2(new __PRIVATE_FirebaseAuthCredentialsProvider(e.getProvider("auth-internal")), new __PRIVATE_FirebaseAppCheckTokenProvider(i, e.getProvider("app-check-internal")), function __PRIVATE_databaseIdFromApp(e, t) {
...
function fromRef$1(ref, options) {
	if (options === void 0) options = DEFAULT_OPTIONS;
	return new Observable(function(subscriber) {
		return { unsubscribe: onSnapshot$1(ref, options, {
...
*/
function doc$1(ref) {
	return fromRef$1(ref, { includeMetadataChanges: true });
}
/**
...
* @param options
*/
function docData$1(ref, options) {
	if (options === void 0) options = {};
	return doc$1(ref).pipe(map(function(snap) {
		return snapToData$1(snap, options);
	}));
}
function snapToData$1(snapshot, options) {
	var _a;
	if (options === void 0) options = {};
	var data = snapshot.data(options);
	if (!snapshot.exists() || typeof data !== "object" || data === null || !options.idField) return data;
...
function processIndividualChange(combined, change) {
	switch (change.type) {
		case "added":
			if (combined[change.newIndex] && refEqual$1(combined[change.newIndex].doc.ref, change.doc.ref));
			else return sliceAndSplice(combined, change.newIndex, 0, change);
			break;
		case "modified":
			if (combined[change.oldIndex] == null || refEqual$1(combined[change.oldIndex].doc.ref, change.doc.ref)) if (change.oldIndex !== change.newIndex) {
				var copiedArray = combined.slice();
				copiedArray.splice(change.oldIndex, 1);
				copiedArray.splice(change.newIndex, 0, change);
				return copiedArray;
			} else return sliceAndSplice(combined, change.newIndex, 1, change);
			break;
		case "removed":
			if (combined[change.oldIndex] && refEqual$1(combined[change.oldIndex].doc.ref, change.doc.ref)) return sliceAndSplice(combined, change.oldIndex, 1);
			break;
	}
	return combined;
}
...
function processDocumentChanges(current, changes, events) {
	...
	changes.forEach(function(change) {
		if (events.indexOf(change.type) > -1) current = processIndividualChange(current, change);
	});
	return current;
}
/**
...
var filterEmptyUnlessFirst = function() {
	return pipe(windowwise(), filter(function(_a) {
		var prior = _a[0];
		return _a[1].length > 0 || prior === void 0;
	}), map(function(_a) {
		return _a[1];
	}));
};
...
* @param query
*/
function collectionChanges$1(query, options) {
	if (options === void 0) options = {};
	return fromRef$1(query, { includeMetadataChanges: true }).pipe(windowwise(), map(function(_a) {
		var priorSnapshot = _a[0], currentSnapshot = _a[1];
		var docChanges = currentSnapshot.docChanges();
		if (priorSnapshot && !metaDataEquals(priorSnapshot, currentSnapshot)) currentSnapshot.docs.forEach(function(currentDocSnapshot, currentIndex) {
			var currentDocChange = docChanges.find(function(c) {
				return refEqual$1(c.doc.ref, currentDocSnapshot.ref);
			});
			if (currentDocChange) {
				if (metaDataEquals(currentDocChange.doc, currentDocSnapshot)) return;
			} else {
				var priorDocSnapshot = priorSnapshot === null || priorSnapshot === void 0 ? void 0 : priorSnapshot.docs.find(function(d) {
					return refEqual$1(d.ref, currentDocSnapshot.ref);
				});
				if (priorDocSnapshot && metaDataEquals(priorDocSnapshot, currentDocSnapshot)) return;
			}
			docChanges.push({
...
* @param query
*/
function collection$1(query) {
	return fromRef$1(query, { includeMetadataChanges: true }).pipe(map(function(changes) {
		return changes.docs;
	}));
}
/**
...
* @param query
*/
function sortedChanges$1(query, options) {
	if (options === void 0) options = {};
	return collectionChanges$1(query, options).pipe(scan(function(current, changes) {
...
*/
function auditTrail$1(query, options) {
	if (options === void 0) options = {};
	return collectionChanges$1(query, options).pipe(scan(function(current, action) {
...
* @param options
*/
function collectionData$1(query, options) {
	if (options === void 0) options = {};
	return collection$1(query).pipe(map(function(arr) {
		return arr.map(function(snap) {
			return snapToData$1(snap, options);
		});
	}));
}
function collectionCountSnap$1(query) {
	return from(getCountFromServer$1(query));
}
function collectionCount$1(query) {
	return collectionCountSnap$1(query).pipe(map(function(snap) {
		return snap.data().count;
	}));
}
//#endregion
//#region node_modules/@angular/fire/fesm2022/angular-fire-firestore.mjs
...
function firestoreInstanceFactory(fn) {
	return (zone, injector) => {
		return new Firestore(zone.runOutsideAngular(() => fn(injector)));
	};
}
...
_defineProperty(FirestoreModule, "ɵfac", function FirestoreModule_Factory(__ngFactoryType__) {
	return new (__ngFactoryType__ || _FirestoreModule)();
});
_defineProperty(FirestoreModule, "ɵmod", /* @__PURE__ */ ɵɵdefineNgModule({ type: _FirestoreModule }));
_defineProperty(FirestoreModule, "ɵinj", /* @__PURE__ */ ɵɵdefineInjector({ providers: [DEFAULT_FIRESTORE_INSTANCE_PROVIDER, FIRESTORE_INSTANCES_PROVIDER] }));
(() => {
	(typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FirestoreModule, [{
		type: NgModule,
		args: [{ providers: [DEFAULT_FIRESTORE_INSTANCE_PROVIDER, FIRESTORE_INSTANCES_PROVIDER] }]
	}], () => [], null);
})();
...
var auditTrail = ɵzoneWrap(auditTrail$1, true);
var collectionSnapshots = ɵzoneWrap(collection$1, true);
var collectionChanges = ɵzoneWrap(collectionChanges$1, true);
var collectionCount = ɵzoneWrap(collectionCount$1, true);
var collectionCountSnap = ɵzoneWrap(collectionCountSnap$1, true);
var collectionData = ɵzoneWrap(collectionData$1, true);
var docSnapshots = ɵzoneWrap(doc$1, true);
var docData = ɵzoneWrap(docData$1, true);
var fromRef = ɵzoneWrap(fromRef$1, true);
var snapToData = ɵzoneWrap(snapToData$1, true);
var sortedChanges = ɵzoneWrap(sortedChanges$1, true);
var addDoc = ɵzoneWrap(addDoc$1, true, 2);
var aggregateFieldEqual = ɵzoneWrap(aggregateFieldEqual$1, true, 2);
var aggregateQuerySnapshotEqual = ɵzoneWrap(aggregateQuerySnapshotEqual$1, true, 2);
var and = ɵzoneWrap(and$1, true, 2);
var clearIndexedDbPersistence = ɵzoneWrap(clearIndexedDbPersistence$1, true);
var collection = ɵzoneWrap(collection$2, true, 2);
var collectionGroup = ɵzoneWrap(collectionGroup$1, true, 2);
var connectFirestoreEmulator = ɵzoneWrap(connectFirestoreEmulator$1, true);
var deleteAllPersistentCacheIndexes = ɵzoneWrap(deleteAllPersistentCacheIndexes$1, true);
var deleteDoc = ɵzoneWrap(deleteDoc$1, true, 2);
var deleteField = ɵzoneWrap(deleteField$1, true, 2);
var disableNetwork = ɵzoneWrap(disableNetwork$1, true);
var disablePersistentCacheIndexAutoCreation = ɵzoneWrap(disablePersistentCacheIndexAutoCreation$1, true);
var doc = ɵzoneWrap(doc$2, true, 2);
var documentId = ɵzoneWrap(documentId$1, true, 2);
var enableIndexedDbPersistence = ɵzoneWrap(enableIndexedDbPersistence$1, true);
var enableMultiTabIndexedDbPersistence = ɵzoneWrap(enableMultiTabIndexedDbPersistence$1, true);
var enableNetwork = ɵzoneWrap(enableNetwork$1, true);
var enablePersistentCacheIndexAutoCreation = ɵzoneWrap(enablePersistentCacheIndexAutoCreation$1, true);
var endAt = ɵzoneWrap(endAt$1, true, 2);
var endBefore = ɵzoneWrap(endBefore$1, true, 2);
var getAggregateFromServer = ɵzoneWrap(getAggregateFromServer$1, true);
var getCountFromServer = ɵzoneWrap(getCountFromServer$1, true);
var getDoc = ɵzoneWrap(getDoc$1, true);
var getDocFromCache = ɵzoneWrap(getDocFromCache$1, true);
var getDocFromServer = ɵzoneWrap(getDocFromServer$1, true);
var getDocs = ɵzoneWrap(getDocs$1, true);
var getDocsFromCache = ɵzoneWrap(getDocsFromCache$1, true);
var getDocsFromServer = ɵzoneWrap(getDocsFromServer$1, true);
var getFirestore = ɵzoneWrap(getFirestore$1, true);
var getPersistentCacheIndexManager = ɵzoneWrap(getPersistentCacheIndexManager$1, true);
var increment = ɵzoneWrap(increment$1, true, 2);
var initializeFirestore = ɵzoneWrap(initializeFirestore$1, true);
var limit = ɵzoneWrap(limit$1, true, 2);
var limitToLast = ɵzoneWrap(limitToLast$1, true, 2);
var loadBundle = ɵzoneWrap(loadBundle$1, true);
var namedQuery = ɵzoneWrap(namedQuery$1, true, 2);
var onSnapshot = ɵzoneWrap(onSnapshot$1, true);
var onSnapshotsInSync = ɵzoneWrap(onSnapshotsInSync$1, true);
var or = ɵzoneWrap(or$1, true, 2);
var orderBy = ɵzoneWrap(orderBy$1, true, 2);
var query = ɵzoneWrap(query$1, true, 2);
var queryEqual = ɵzoneWrap(queryEqual$1, true, 2);
var refEqual = ɵzoneWrap(refEqual$1, true, 2);
var runTransaction = ɵzoneWrap(runTransaction$1, true);
var setDoc = ɵzoneWrap(setDoc$1, true, 2);
var setIndexConfiguration = ɵzoneWrap(setIndexConfiguration$1, true);
var setLogLevel = ɵzoneWrap(setLogLevel$1, true);
var snapshotEqual = ɵzoneWrap(snapshotEqual$1, true, 2);
var startAfter = ɵzoneWrap(startAfter$1, true, 2);
var startAt = ɵzoneWrap(startAt$1, true, 2);
var sum = ɵzoneWrap(sum$1, true, 2);
var terminate = ɵzoneWrap(terminate$1, true);
var updateDoc = ɵzoneWrap(updateDoc$1, true, 2);
var vector = ɵzoneWrap(vector$1, true, 2);
var waitForPendingWrites = ɵzoneWrap(waitForPendingWrites$1, true);
var where = ɵzoneWrap(where$1, true, 2);
var writeBatch = ɵzoneWrap(writeBatch$1, true, 2);
//#endregion
export { AbstractUserDataWriter, AggregateField, AggregateQuerySnapshot, Bytes, un as CACHE_SIZE_UNLIMITED, CollectionReference, DocumentReference, DocumentSnapshot, FieldPath, FieldValue, Firestore, FirestoreError, FirestoreInstances, FirestoreModule, GeoPoint, LoadBundleTask, PersistentCacheIndexManager, Query, QueryCompositeFilterConstraint, QueryConstraint, QueryDocumentSnapshot, QueryEndAtConstraint, QueryFieldFilterConstraint, QueryLimitConstraint, QueryOrderByConstraint, QuerySnapshot, QueryStartAtConstraint, SnapshotMetadata, Timestamp, Transaction, VectorValue, WriteBatch, __PRIVATE_AutoId as _AutoId, ByteString as _ByteString, DatabaseId as _DatabaseId, DocumentKey as _DocumentKey, __PRIVATE_EmptyAppCheckTokenProvider as _EmptyAppCheckTokenProvider, __PRIVATE_EmptyAuthCredentialsProvider as _EmptyAuthCredentialsProvider, FieldPath$1 as _FieldPath, TestingHooks as _TestingHooks, __PRIVATE_cast as _cast, __PRIVATE_debugAssert as _debugAssert, _internalAggregationQueryToProtoRunAggregationQueryRequest, _internalQueryToProtoQueryTarget, __PRIVATE_isBase64Available as _isBase64Available, __PRIVATE_logWarn as _logWarn, __PRIVATE_validateIsNotUsedTogether as _validateIsNotUsedTogether, addDoc, aggregateFieldEqual, aggregateQuerySnapshotEqual, and, arrayRemove, arrayUnion, auditTrail, average, clearIndexedDbPersistence, collection, collectionChanges, collectionCount, collectionCountSnap, collectionData, collectionGroup, collectionSnapshots, connectFirestoreEmulator, count, deleteAllPersistentCacheIndexes, deleteDoc, deleteField, disableNetwork, disablePersistentCacheIndexAutoCreation, doc, docData, docSnapshots, documentId, documentSnapshotFromJSON, enableIndexedDbPersistence, enableMultiTabIndexedDbPersistence, enableNetwork, enablePersistentCacheIndexAutoCreation, endAt, endBefore, ensureFirestoreConfigured, executeWrite, firestoreInstance$, fromRef, getAggregateFromServer, getCountFromServer, getDoc, getDocFromCache, getDocFromServer, getDocs, getDocsFromCache, getDocsFromServer, getFirestore, getPersistentCacheIndexManager, increment, initializeFirestore, limit, limitToLast, loadBundle, memoryEagerGarbageCollector, memoryLocalCache, memoryLruGarbageCollector, namedQuery, onSnapshot, onSnapshotResume, onSnapshotsInSync, or, orderBy, persistentLocalCache, persistentMultipleTabManager, persistentSingleTabManager, provideFirestore, query, queryEqual, querySnapshotFromJSON, refEqual, runTransaction, serverTimestamp, setDoc, setIndexConfiguration, setLogLevel, snapToData, snapshotEqual, sortedChanges, startAfter, startAt, sum, terminate, updateDoc, vector, waitForPendingWrites, where, writeBatch };
