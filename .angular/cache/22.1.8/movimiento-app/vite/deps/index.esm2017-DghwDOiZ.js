...
function _pingServer() {
	_pingServer = _asyncToGenerator(function* (endpoint) {
		return (yield fetch(endpoint, { credentials: "include" })).ok;
	});
	return _pingServer.apply(this, arguments);
}
/**
...
function _readHeartbeatsFromIndexedDB() {
	_readHeartbeatsFromIndexedDB = _asyncToGenerator(function* (app) {
		try {
			const tx = (yield getDbPromise()).transaction(STORE_NAME);
...
function _writeHeartbeatsToIndexedDB() {
	_writeHeartbeatsToIndexedDB = _asyncToGenerator(function* (app, heartbeatObject) {
		try {
			const tx = (yield getDbPromise()).transaction(STORE_NAME, "readwrite");
			yield tx.objectStore(STORE_NAME).put(heartbeatObject, computeKey(app));
			yield tx.done;
		} catch (e) {
			if (e instanceof FirebaseError) logger.warn(e.message);
			else {
	...
	*/
	triggerHeartbeat() {
		...
		return _asyncToGenerator(function* () {
			var _a, _b;
			try {
				const agent = _this.container.getProvider("platform-logger").getImmediate().getPlatformInfoString();
				...
				if (((_a = _this._heartbeatsCache) === null || _a === void 0 ? void 0 : _a.heartbeats) == null) {
					...
					if (((_b = _this._heartbeatsCache) === null || _b === void 0 ? void 0 : _b.heartbeats) == null) return;
				}
				if (_this._heartbeatsCache.lastSentHeartbeatDate === date || _this._heartbeatsCache.heartbeats.some((singleDateHeartbeat) => singleDateHeartbeat.date === date)) return;
				else {
	...
	*/
	getHeartbeatsHeader() {
		...
		return _asyncToGenerator(function* () {
			var _a;
			try {
				if (_this2._heartbeatsCache === null) yield _this2._heartbeatsCachePromise;
				if (((_a = _this2._heartbeatsCache) === null || _a === void 0 ? void 0 : _a.heartbeats) == null || _this2._heartbeatsCache.heartbeats.length === 0) return "";
				const date = getUTCDateString();
				const { heartbeatsToSend, unsentEntries } = extractHeartbeatsForHeader(_this2._heartbeatsCache.heartbeats);
...
var HeartbeatStorageImpl = class {
	...
	*/
	read() {
		var _this3 = this;
		return _asyncToGenerator(function* () {
			if (!(yield _this3._canUseIndexedDBPromise)) return { heartbeats: [] };
			else {
	...
	overwrite(heartbeatsObject) {
		...
		return _asyncToGenerator(function* () {
			var _a;
			if (!(yield _this4._canUseIndexedDBPromise)) return;
			else {
	...
	add(heartbeatsObject) {
		...
		return _asyncToGenerator(function* () {
			var _a;
			if (!(yield _this5._canUseIndexedDBPromise)) return;
			else {
...
//#endregion
export { querystring as $, calculateBackoffMillis as A, getUA as B, Logger as C, FirebaseError as D, ErrorFactory as E, getDefaultEmulatorHost as F, isIE as G, isCloudWorkstation as H, getDefaultEmulatorHostnameAndPort as I, isReactNative as J, isIndexedDBAvailable as K, getExperimentalSetting as L, createSubscribe as M, deepEqual as N, base64 as O, extractQuerystring as P, pingServer as Q, getGlobal as R, LogLevel as S, Deferred as T, isCloudflareWorker as U, isBrowserExtension as V, isEmpty as W, isSafariOrWebkit as X, isSafari as Y, issuedAtTime as Z, initializeApp as _, _apps as a, registerVersion as b, _getProvider as c, _registerComponent as d, querystringDecode as et, _removeServiceInstance as f, getApps as g, getApp as h, _addOrOverwriteComponent as i, createMockUserToken as j, base64Decode as k, _isFirebaseApp as l, deleteApp as m, SDK_VERSION as n, _clearComponents as o, _serverApps as p, isMobileCordova as q, _addComponent as r, _components as s, DEFAULT_ENTRY_NAME as t, updateEmulatorBanner as tt, _isFirebaseServerApp as u, initializeServerApp as v, Component as w, setLogLevel as x, onLog as y, getModularInstance as z };
